...
public interface VoucherRepository extends JpaRepository<Voucher, Long> {
    ...
    @Query("SELECT new com.dxerp.ebs.dto.VoucherDTO(v.id, v.voucherNo, v.voucherDate, v.totalAmount, v.type, v.status, v.narration) " +
           "FROM Voucher v WHERE v.voucherDate BETWEEN :startDate AND :endDate")
    List<VoucherDTO> findByVoucherDateBetween(Date startDate, Date endDate);
    ...
    @Query("SELECT new com.dxerp.ebs.dto.VoucherDTO(v.id, v.voucherNo, v.voucherDate, v.totalAmount, v.type, v.status, v.narration) FROM Voucher v")
